<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}'>学院分类：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.xueyuanfenlei}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','xueyuanfenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','xueyuanfenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('xueyuanfenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussxueyuanfenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('xueyuanfenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('xueyuanfenlei', this.detail);
				this.$utils.msg('审核成功');
