<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}'>专业分类：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.zhuanyefenlei}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zhuanyefenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','zhuanyefenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('zhuanyefenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusszhuanyefenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('zhuanyefenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('zhuanyefenlei', this.detail);
				this.$utils.msg('审核成功');
